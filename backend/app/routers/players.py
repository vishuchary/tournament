...
def _cross_group_rank(groups: list, fmt: str) -> list[dict]:
    """Rank all teams across multiple groups using pre-computed or live standings."""
    all_entries = []
    for group in groups:
...
def get_player_stats(name: str):
    try:
        ...
        for t_doc in db.collection('tournaments').stream():
            ...
            # Determine winner/runner-up from last level
            if levels:
                last = levels[-1]
        ...
        # Process competitive matches
        for m_doc in db.collection('competitive_matches').stream():
            m = m_doc.to_dict() or {}
